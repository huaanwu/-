...
const DOMAINS = {
  ...
  'Paper':     ['init', 'buy', 'sell', 'getAccount', 'getPositions', 'resetAccount', 'snapshotIfNeeded', 'autoTradeFromPick', 'renderPage', 'buyFromForm', 'sellFromForm', 'sellAll', 'switchSleeve', '_getAccountRaw', '_saveAccountRaw', '_calcFee', '_roundLot', '_pushSnapshot', '_planAutoTrade', 'maybeGenerateEodReport', '_shouldGenerateEod', '_pushEodReport', '_appendDisciplineLog', '_logDisciplineBlock', '_buildEodReport', '_formatEodReportText', '_pushEodToFeishu', '_renderEodReport', 'addCondOrder', 'listCondOrders', 'cancelCondOrder', 'settleCondOrders', '_checkCondOrder', '_barOf', '_lastClosedBar', '_orderEligible', '_tradingDaysAfter', '_isGapDown', '_isGapUp', '_fillCheck', '_exitCheck', '_settleFill', '_settleExit', '_writeCondJournal', '_condPlanLines', 'addCondOrderFromForm', '_renderCondOrders',
    // T2 盘前 AI 计划
    'generateMorningPlan', 'confirmMorningCandidates', 'cancelMorningPlan', 'listMorningPlans',
    '_pushMorningPlan', '_cleanCandidate', '_parsePlanJson', '_buildT2Context', '_saveMorningPlan', '_renderPrePlan',
    // T4 短线学习环
    '_severityWeight', '_sortLessonsByScore', '_formatLessonsForPrompt', '_cleanLesson', '_pushShortLesson',
    '_shouldRunShortLearning', '_buildT4Context', 'listShortLessons', '_saveShortLessons',
    // T4 主入口 (T4 commit 才会接通 UI, 这里只检查定义)
    'maybeRunShortLearningLoop', 'runShortLearningLoop', '_renderShortLessons'],
...
// ========== [38.3] T2 盘前 AI 计划 (Paper) ==========
(async () => {
  try {
    const pad2T2 = (n) => String(n).padStart(2, '0');
    const fmtDateT2 = (d) => {
      d = d instanceof Date ? d : new Date(d);
      return `${d.getFullYear()}-${pad2T2(d.getMonth() + 1)}-${pad2T2(d.getDate())}`;
    };
    // 38 节 paperSrc / K 是 IIFE 局部, 这里独立读
    const paperSrc = readFileSafe(path.join(WWW, 'app', 'paper.js'));
    const K = (() => {
      const kctx = { window: {}, console };
      kctx.window.Core = kctx.window.Core || {};
      vm.createContext(kctx);
      vm.runInContext(fs.readFileSync('www/core/constants.js', 'utf-8'), kctx);
      return kctx.window.Core.Constants;
    })();
    const buildCtx = (storageData, opts = {}) => {
      const pctx = {
        window: {}, console,
        ...
        pctClass: () => '',
        fmtDate: fmtDateT2,
        uuid: () => 't2-' + Math.random().toString(36).slice(2, 10),
        parseStockInput: (t) => {
          const m = String(t || '').trim().match(/^(\d{6})/);
          return m ? { code: m[1], name: String(t).slice(6).trim() } : null;
        },
      ...
      pctx.window.Core = {
        ...
        Constants: K,
        Discipline: {
          preBuyCheck: async () => ({ ok: !(opts.disciplineBlocks && opts.disciplineBlocks.length), blocks: opts.disciplineBlocks || [], warns: [], history: [] }),
          renderCheckResult: () => '', _resultToText: () => ''
        },
        MarketWidth: { snapshot: async () => null }
      };
      pctx.Core = pctx.window.Core;
      pctx.window.document = { getElementById: () => null };
      pctx.document = pctx.window.document;
    ...
    // 38.3.a _pushMorningPlan 同日去重 + 上限
    const ctx_a = buildCtx({ kv: {}, tables: {} });
    const PA = ctx_a.window.Paper;
    let pl = [];
    pl = PA._pushMorningPlan(pl, { date: '2026-07-25', summary: 'a' });
    pl = PA._pushMorningPlan(pl, { date: '2026-07-25', summary: 'a-dup' });  // 同日覆盖-不增加
    pl = PA._pushMorningPlan(pl, { date: '2026-07-26', summary: 'b' });
    if (pl.length === 2 && pl[0].date === '2026-07-25' && pl[1].date === '2026-07-26') ok('T2._pushMorningPlan: 同日去重');
    else fail('T2._pushMorningPlan 同日去重', JSON.stringify(pl));

    // 上限 10 滚动截断
    let pl2 = [];
    for (let i = 0; i < 12; i++) pl2 = PA._pushMorningPlan(pl2, { date: `2026-08-${String(i+1).padStart(2,'0')}`, summary: 'x' });
    if (pl2.length === 10 && pl2[0].date === '2026-08-03' && pl2[9].date === '2026-08-12') ok('T2._pushMorningPlan: 上限 10 滚动截断');
    else fail('T2._pushMorningPlan 上限', 'len=' + pl2.length);

    // 38.3.b _cleanCandidate 价格关系校验
    const ctx_b = buildCtx({ kv: {}, tables: {} });
    const PB = ctx_b.window.Paper;
    // 短线现金 30000, 单价调到 50 让学生可买得起
    const good = PB._cleanCandidate({
      code: '600519', name: '贵州茅台', direction: 'below',
      triggerPrice: 50, stopLoss: 46, targetPrice: 58,
      shares: 200, assumption: '业绩拐点', reasoning: '回调到支撑位'
    }, 30000);
    if (good.cleaned && good.cleaned.code === '600519' && good.cleaned.shares === 200 && good.cleaned.assumption === '业绩拐点') ok('T2._cleanCandidate: 合法候选');
    else fail('T2._cleanCandidate 合法', JSON.stringify(good));

    const badPrice = PB._cleanCandidate({
      code: '600519', direction: 'below',
      triggerPrice: 50, stopLoss: 52, targetPrice: 58,  // stopLoss > triggerPrice 错
      shares: 100
    }, 30000);
    if (badPrice.cleaned === null && badPrice.errs.length > 0) ok('T2._cleanCandidate: 价格关系错拒收');
    else fail('T2._cleanCandidate 价格关系', JSON.stringify(badPrice));

    const badShares = PB._cleanCandidate({
      code: '600519', direction: 'above',
      triggerPrice: 50, stopLoss: 46, targetPrice: 58,
      shares: 50  // 不足一手
    }, 30000);
    if (badShares.cleaned === null) ok('T2._cleanCandidate: 不足一手拒收');
    else fail('T2._cleanCandidate 不足一手', JSON.stringify(badShares));

    // 38.3.c _parsePlanJson 兼容 markdown 围栏
    const ctx_c = buildCtx({ kv: {}, tables: {} });
    const PC = ctx_c.window.Paper;
    const json1 = PC._parsePlanJson('{"summary":"今日看空", "candidates":[]}');
    if (json1 && json1.summary === '今日看空' && Array.isArray(json1.candidates)) ok('T2._parsePlanJson: 纯 JSON');
    else fail('T2._parsePlanJson 纯 JSON', JSON.stringify(json1));

    const json2 = PC._parsePlanJson('```json\n{"summary":"fenced", "candidates":[]}\n```');
    if (json2 && json2.summary === 'fenced') ok('T2._parsePlanJson: markdown 围栏');
    else fail('T2._parsePlanJson 围栏', JSON.stringify(json2));

    const json3 = PC._parsePlanJson('前缀废话 {"summary":"s", "candidates":[]} 后缀废话');
    if (json3 && json3.summary === 's') ok('T2._parsePlanJson: 杂文本抽取');
    else fail('T2._parsePlanJson 杂文本', JSON.stringify(json3));

    const jsonBad = PC._parsePlanJson('not json at all');
    if (jsonBad === null) ok('T2._parsePlanJson: 解析失败返 null');
    else fail('T2._parsePlanJson 失败', JSON.stringify(jsonBad));

    // 38.3.d _saveMorningPlan 写入 + 同日覆盖
    const ctx_d = buildCtx({ kv: {}, tables: {} });
    const PD = ctx_d.window.Paper;
    await PD._saveMorningPlan({ date: '2026-07-27', summary: 'first', candidates: [{code:'600519', direction:'below'}] });
    await PD._saveMorningPlan({ date: '2026-07-27', summary: 'second', candidates: [] });
    const list = await PD.listMorningPlans(10);
    if (list.length === 1 && list[0].summary === 'second') ok('T2._saveMorningPlan: 同日覆盖');
    else fail('T2._saveMorningPlan 覆盖', JSON.stringify(list));

    // 38.3.e _buildT2Context 空仓空单空 lessons 不报错
    const ctx_e = buildCtx({ kv: {}, tables: {} });
    const PE = ctx_e.window.Paper;
    const t2ctx = await PE._buildT2Context();
    if (t2ctx && t2ctx.cash === 30000 && Array.isArray(t2ctx.positions) && Array.isArray(t2ctx.lessons)) ok('T2._buildT2Context: 默认 30000 + 空数组');
    else fail('T2._buildT2Context', JSON.stringify(t2ctx));

  } catch (e) {
    fail('T2 盘前 AI 计划', e.message + ' / ' + (e.stack || ''));
  }
})();
...
(async () => {
  try {
    const pad2T4 = (n) => String(n).padStart(2, '0');
    const fmtDateT4 = (d) => {
      d = d instanceof Date ? d : new Date(d);
      return `${d.getFullYear()}-${pad2T4(d.getMonth() + 1)}-${pad2T4(d.getDate())}`;
    };
    const paperSrc = readFileSafe(path.join(WWW, 'app', 'paper.js'));
    const K = (() => {
      const kctx = { window: {}, console };
    ...
    const buildCtx = (storageData, opts = {}) => {
      const pctx = {
        ...
        pctClass: () => '',
        fmtDate: fmtDateT4,
        uuid: () => 't4-' + Math.random().toString(36).slice(2, 10),
        parseStockInput: (t) => {
          const m = String(t || '').trim().match(/^(\d{6})/);
          return m ? { code: m[1], name: String(t).slice(6).trim() } : null;
        },
      ...
      pctx.window.Core = {
        ...
        Constants: K,
        Discipline: {
          preBuyCheck: async () => ({ ok: true, blocks: [], warns: [], history: [] }),
          renderCheckResult: () => '', _resultToText: () => ''
        }
      };
      pctx.Core = pctx.window.Core;
    ...
    // 38.4.a _severityWeight 权重映射
    const ctx_a = buildCtx({ kv: {}, tables: {} });
    const PA = ctx_a.window.Paper;
    if (PA._severityWeight('critical') === 9 && PA._severityWeight('warning') === 5 && PA._severityWeight('info') === 2) ok('T4._severityWeight: 权重映射');
    else fail('T4._severityWeight', JSON.stringify([PA._severityWeight('critical'), PA._severityWeight('warning'), PA._severityWeight('info')]));

    // 38.4.b _sortLessonsByScore 排序正确
    const sorted = PA._sortLessonsByScore([
      { severity: 'info', dateMs: Date.now() - 3 * 86400000, lesson: 'i' },
      { severity: 'critical', dateMs: Date.now() - 1 * 86400000, lesson: 'c' },
      { severity: 'warning', dateMs: Date.now() - 2 * 86400000, lesson: 'w' }
    ]);
    if (sorted[0].severity === 'critical' && sorted[1].severity === 'warning' && sorted[2].severity === 'info') ok('T4._sortLessonsByScore: critical > warning > info');
    else fail('T4._sortLessonsByScore', sorted.map(s => s.severity).join(','));

    // 38.4.c _formatLessonsForPrompt 自然语言列表
    const formatted = PA._formatLessonsForPrompt([
      { date: '2026-07-22', category: 'exit-timing', severity: 'warning', lesson: '跌破止损后拖延', refCode: '600519' }
    ], 5);
    if (Array.isArray(formatted) && formatted.length === 1
        && /2026-07-22/.test(formatted[0])
        && /warning/.test(formatted[0])
        && /600519/.test(formatted[0])) ok('T4._formatLessonsForPrompt: 自然语言格式');
    else fail('T4._formatLessonsForPrompt', JSON.stringify(formatted));

    // 38.4.d _cleanLesson 白名单 + scope 强制 'short'
    const cleaned = PA._cleanLesson({
      date: '2026-07-22', category: 'assumption', severity: 'critical',
      lesson: '假设错的', refCode: '600519'
    }, '2026-07-22');
    if (cleaned.scope === 'short' && cleaned.category === 'assumption' && cleaned.severity === 'critical' && cleaned.refCode === '600519') ok('T4._cleanLesson: 字段白名单 + scope=short');
    else fail('T4._cleanLesson', JSON.stringify(cleaned));

    // 非法 category/severity 落 'other'/'info'
    const cleanedBad = PA._cleanLesson({
      date: '2026-07-22', category: 'haha-fake', severity: 'fake',
      lesson: 'x', refCode: 'abc'  // 非 6 位
    }, '2026-07-22');
    if (cleanedBad.category === 'other' && cleanedBad.severity === 'info' && cleanedBad.refCode === '') ok('T4._cleanLesson: 非法值兜底');
    else fail('T4._cleanLesson 兜底', JSON.stringify(cleanedBad));

    // 38.4.e _shouldRunShortLearning 工作日 + 15:30 后 + 未跑过
    if (PA._shouldRunShortLearning(new Date(2026, 6, 27, 15, 31), null) === true) ok('T4._shouldRunShortLearning: 工作日 15:31 + 未跑 → true');
    else fail('T4._shouldRunShortLearning 工作日 15:31', 'should be true');
    if (PA._shouldRunShortLearning(new Date(2026, 6, 27, 15, 29), null) === false) ok('T4._shouldRunShortLearning: 15:29 前 → false');
    else fail('T4._shouldRunShortLearning 15:29', 'should be false');
    if (PA._shouldRunShortLearning(new Date(2026, 6, 26, 16, 0), '2026-07-26') === false) ok('T4._shouldRunShortLearning: 周末 → false');
    else fail('T4._shouldRunShortLearning 周末', 'should be false');
    if (PA._shouldRunShortLearning(new Date(2026, 6, 27, 16, 0), '2026-07-27') === false) ok('T4._shouldRunShortLearning: 今日已跑 → false');
    else fail('T4._shouldRunShortLearning 已跑', 'should be false');

    // 38.4.f _pushShortLesson 上限 50
    let ls = [];
    for (let i = 0; i < 55; i++) ls = PA._pushShortLesson(ls, { date: `2026-07-${String((i % 30) + 1).padStart(2, '0')}`, lesson: 'l' + i, severity: 'info' });
    if (ls.length === 50) ok('T4._pushShortLesson: 上限 50');
    else fail('T4._pushShortLesson 上限', 'len=' + ls.length);

    // 38.4.g _saveShortLessons 落库 + 同日覆写规则 (按 score 排序后)
    const ctx_g = buildCtx({ kv: {}, tables: {} });
    const PG = ctx_g.window.Paper;
    await PG._saveShortLessons([
      { date: '2026-07-25', category: 'assumption', severity: 'info', lesson: 'inf' },
      { date: '2026-07-26', category: 'exit-timing', severity: 'critical', lesson: 'crit' }
    ]);
    const read = await PG.listShortLessons(10);
    if (read.length === 2 && read[0].severity === 'critical' && read[1].severity === 'info') ok('T4._saveShortLessons + list: 落库按 score 排序');
    else fail('T4._saveShortLessons', JSON.stringify(read));
